"""
Pydantic schemas for Super Koch (Osuper platform) GraphQL responses.
...
Usage:
    ...
    try:
        product = SuperKochProduct.parse_obj(normalized_data)
...
class SuperKochProduct(BaseModel):
    """
    Super Koch product data model (normalized from GraphQL).

    Fields are normalized to match common e-commerce patterns,
    making integration with DBT models easier.
    """
    # Core identifiers
    productId: str = Field(..., min_length=1, description="Product ID from Super Koch")
